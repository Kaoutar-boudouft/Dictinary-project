...
if(!empty($_POST['word'])){
    $mot=$_POST['word'];
    $user=$_POST['user'];
    $result=DictManager::search($mot,$user);
    echo("</br>");     
    if($result!="vous n'avez pas le droit de saisir des nombres :)!" && $result!="Vous Devez Inserez qq chose !"){
      
      $f=$result->columnCount()-2;
      if($result->rowCount()==0){
        if(preg_match("/[a-z]/i", $mot)){
          echo("<h6>Ce mot n'existe pas dans votre Dictionnaire ! <a href='https://translate.google.com/?hl=fr&sl=fr&tl=ar&text=$mot&op=translate' target='_blank' style='color: black;font-size:14pt'>Le rechercher maintenant!</a></h6>");        
        }
        else{
          echo("<h6>Ce mot n'existe pas dans votre Dictionnaire ! <a href='https://translate.google.com/?hl=fr&sl=ar&tl=fr&text=$mot&op=translate' target='_blank' style='color: black;font-size:14pt'>Le rechercher maintenant!</a></h6>");        
        }
      }
      else{
        ...
        echo("<th scope='col' style='border:1px solid rgba(149, 64, 219, 0.91);padding:8px;text-align:center'></th>");
